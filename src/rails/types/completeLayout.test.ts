...
import {
  ...
  MockLayoutStraight3,
  MockLayoutCircle360MixB,
  MockLayoutCircle90x4reverse,
  MockLayoutOval,
...
describe("完全レイアウトを判定できるか？", () => {
  ...
  //   ).toBe(true);
  //   expect(
  //     areCompleteLayout(SIMULATOR_RAIL_CONSTANTS, MockLayoutCircle360MixB)
  //   ).toBe(true);
  // });
  test("内回りの曲線レール", () => {
    expect(
      areCompleteLayout(SIMULATOR_RAIL_CONSTANTS, MockLayoutCircle90x4reverse)
    ).toBe(true);
  });
});
...
describe("直線と曲線によるレイアウトを完全判定できるか？", () => {
  test("オーバル。曲線の中に合計2本の直線があるレイアウト", () => {
    expect(areCompleteLayout(SIMULATOR_RAIL_CONSTANTS, MockLayoutOval)).toBe(
      true
    );
    expect(
      areCompleteLayout(SIMULATOR_RAIL_CONSTANTS, [
